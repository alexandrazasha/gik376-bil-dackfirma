<script setup>
// Importerar funktioner från Vue och vår bookingService
...
// Hämtar alla bokningar från vår service-fil
const allBookings = getBookings()

// Reaktiva variabler för att hantera sökning och filter
const searchQuery = ref('')
const statusFilter = ref('alla')
const dateFilter = ref('') // tomt = alla datum

// Håller koll på vilken bokning som är expanderad för "Läs mer"
const expandedId = ref(null)

// Funktion för att visa/dölja extra information om en bokning
const toggleReadMore = (id) => {
  expandedId.value = expandedId.value === id ? null : id
}

// Objekt med beskrivningar för de olika tjänsterna
const serviceDescriptions = {
  ...
  'Oljebyte': 'Byte av motorolja och oljefilter för att säkerställa motorns livslängd.',
  'Bromsjustering': 'Kontroll och justering av bromssystemet för optimal säkerhet.',
  'Övrigt': 'Specialarbeten enligt överenskommelse med kund.'
}

// Funktion för att hämta beskrivningen för en specifik tjänst
...
// Variabler för att hantera redigering av en bokning
const editingId = ref(null)
const editForm = ref({ kundNamn: '', regNr: '', service: '' })

// Startar redigeringsläget för en bokning
const startEdit = (booking) => {
  editingId.value = booking.id
  editForm.value = { ...booking } // Skapar en kopia för att inte ändra originalet direkt
}
...
// Sparar ändringarna och stänger redigeringsläget
const saveEdit = () => {
  updateBooking(editingId.value, editForm.value)
  editingId.value = null
}

// Visar en bekräftelseruta innan en bokning tas bort
...
// Computed property för att filtrera bokningar baserat på sökfrågan
const searchedBookings = computed(() => {
  const query = searchQuery.value.trim().toLowerCase()
  // Om sökfältet är tomt, visa alla bokningar
  if (!query) return allBookings.value

  // Annars, filtrera listan
...
// Computed property som filtrerar listan baserat på status och datum
const filteredBookings = computed(() => {
  // Börjar med den sök-filtrerade listan
  let list = searchedBookings.value

  // Filtrerar på status om ett filter är valt
  if (statusFilter.value !== 'alla') {
    list = list.filter((b) => b.status === statusFilter.value)
  }

  // Filtrerar på datum om ett datum är valt
  if (dateFilter.value) {
...
<template>
  <section>
    ...
    <div class="controls-container">
      ...
      <!-- Knappar för att filtrera på status -->
    ...
    <!-- Wrapper för själva bokningslistan -->
    <div class="booking-list-wrapper">
      <!-- Meddelande som visas om inga bokningar matchar filtren -->
      <p v-if="filteredBookings.length === 0">
        Hittade inga bokningar som matchar sök- och filterkriterierna.
      </p>

      <!-- Lista som renderas med v-for, en <li> för varje bokning -->
      <ul v-else class="booking-list">
        <li v-for="booking in filteredBookings" :key="booking.id" :class="booking.status">
          ...
          <!-- NORMALLÄGE (visas när man inte redigerar) -->
          <template v-else>
            ...
            <!-- Extra information som visas när man klickar på "Läs mer" -->
